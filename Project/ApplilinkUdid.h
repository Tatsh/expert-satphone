...
/**
 * Writes a freshly generated UDID into the first empty pasteboard slot.
...
/**
 * Writes the given UDID into the first empty pasteboard slot.
...
/**
 * Replaces the pasteboard's slot 0 UDID with the given value.
 * @param udid The UDID to store.
 * @return The stored record, or nil on failure.
...
/**
 * The pasteboard UDID for a service name and storage index.
...
/**
 * Deletes the pasteboard UDID for a service name and storage index.
 * @param serviceName The pasteboard service name.
 * @param storageIndex The storage slot index.
...
/**
 * Deletes every stored pasteboard UDID, current and old.
...
/**
 * Generates, hashes, and persists a fresh advertising reward UDID.
 * @param error Set to a localised error on failure.
...
/**
 * Deletes the advertising reward UDID at a keychain index.
...
/**
 * Deletes every advertising reward UDID keychain entry.
...
/**
 * Stores the old UDID in the keychain under the old service name.
...
/**
 * Reads the old UDID from the keychain.
 * @param error Set to a localised error on failure.
...
/**
 * Deletes the old UDID keychain entry.
 * @param error Set to a localised error on failure.
 * @return @c YES when the entry was deleted or absent.
...
/**
 * Stores the new UDID in the keychain and the SDK core, keyed by the stored ad storage
 * index.
...
/**
 * Adds or updates a keychain generic-password item storing a UDID under a service key.
...
/**
 * Reads and validates a keychain UDID, updating its modification date on read.
...
/**
 * Looks up a single keychain generic-password item by service key.
 * @param service The keychain service key.
...
/**
 * Deletes a keychain generic-password item by service key.
 * @param service The keychain service key.
 * @param error Set to a localised error on failure.
...
/**
 * Validates that a keychain attribute dictionary holds a well-formed, live UDID record.
...
/**
 * Records the account (storage index) for a service key in the keychain.
 * @param service The keychain service key.
...
/**
 * Attaches the resolved UDIDs to a request parameter dictionary, honouring the priority
 * type.
...
/**
 * Attaches the primary UDID to a request parameter dictionary.
 * @param udidParameters The mutable parameter dictionary to populate.
...
/**
 * Copies the pasteboard-stored UDID for the recorded storage index into the keychain.
...
/**
 * Logs the pasteboard and SDK-core UDID state when the debug environment default is set.
