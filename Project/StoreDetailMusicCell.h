...
/**
 * @brief A non-selectable cell describing one downloadable music entry.
 *
...
/**
 * @brief The overlaid "extend" badge image, added directly to the cell.
 */
@property(strong, nonatomic, nullable) UIImageView *extendImg;

/**
 * @brief The controller used to open the iTunes link; held weakly.
 */
...
/**
 * @brief Sets the iTunes link from a URL string and shows or hides the link button accordingly.
...
/**
 * @brief Hides the sample overlay and stops the download spinner.
 * @ghidraAddress 0xfd094
 */
- (void)sampleStop;

/**
 * @brief Shows the sample overlay spinning while the sample downloads.
 * @ghidraAddress 0xfd0e0
 */
- (void)sampleDownloading;

/**
 * @brief Shows the sample overlay with the play badge while the sample plays.
 * @ghidraAddress 0xfd148
 */
- (void)samplePlaying;

/**
 * @brief Presents the "Show in iTunes Store?" confirmation for the link button.
...
/**
 * @brief @c AlertViewManager delegate callback; opens the iTunes link when OK is tapped.
...
/**
 * @brief No-op teardown hook retained from the original.
 * @ghidraAddress 0xfd2b8
 */
- (void)terminate;

/**
 * @brief Closes any alert presented through @c AlertViewManager.
