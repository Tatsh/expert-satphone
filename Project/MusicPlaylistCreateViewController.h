/**
 * @file
 * The screen for naming a new music playlist: a single rounded-rect text field with a
...
/**
 * Receives the name entered on a @c MusicPlaylistCreateViewController .
 */
...
/**
 * Called when the user commits a non-empty playlist name.
 *
...
/**
 * A modal screen that asks the user to name a new music playlist.
 */
@interface MusicPlaylistCreateViewController : UIViewController <UITextFieldDelegate>

/**
 * The object told which name the user entered.
...
/**
 * Builds the controller and its navigation bar.
 *
...
/**
 * Builds the view hierarchy by hand.
 *
...
/**
 * Enables the Done button only while the field is non-empty.
...
/**
 * Commits the entered name and pops the controller.
 *
...
/**
 * Treats the return key as the Done button when the field is non-empty.
...
/**
 * Enforces the maximum name length and always allows a bare newline.
...
/**
 * Lays the field out and asks the system not to extend the layout under the bars.
...
/**
 * Gives the field focus once the screen is on-screen.
 * @param animated Whether the appearance is animated.
...
/**
 * Dismisses the keyboard as the screen leaves.
