...
/**
 * A @c CAGradientLayer -backed licence-agreement sheet.
 *
...
/**
 * The layer class, so the view is backed by a @c CAGradientLayer .
...
/**
 * Designated initialiser: builds the board and starts the licence-date download.
 *
...
/**
 * Reports whether the given key participates in the pad-scaled layout.
...
/**
 * Reports whether the licence stored under the key predates the downloaded current date.
...
/**
 * Builds the gradient board, heading, scrollable policy text, and the two buttons.
...
/**
 * Routes a download error message to the delegate's @c agreementError:msgStr: .
...
/**
 * @c jubeatLabAccess callback for download progress. Empty in the shipped binary.
...
/**
 * @c jubeatLabAccess callback for a failed download; reports the error to the delegate.
 * @param access The licence-access client.
...
/**
 * @c jubeatLabAccess callback for a finished download; parses the response and drives the
 * flow.
 * @param access The licence-access client.
...
/**
 * @c Downloader callback for a finished download; parses the store or challenge response.
...
/**
 * @c Downloader callback for a failed download; reports the error to the delegate.
...
/**
 * @c Downloader callback for download progress. Empty in the shipped binary.
...
/**
 * Persists the agreed date and fades the board out, then notifies the delegate.
...
/**
 * Notifies the delegate that the user declined.
...
/**
 * Enables the Agree button once the policy text is scrolled to the bottom.
...
/**
 * Enables the Agree button when dragging ends at the bottom of the policy text.
 * @param scrollView The scrolling licence text view.
...
/**
 * The view that is dimmed behind the sheet.
