/**
 * @file
 * A table listing the ten difficulty levels used to filter a music playlist.
 *
...
/**
 * Notified when the user picks a difficulty level from the list.
 */
...
/**
 * Tells the delegate which level the user chose.
 *
...
/**
 * A table view controller listing difficulty levels for filtering a music playlist.
 */
@interface MusicPlaylistLevelSelector : UITableViewController

/**
 * The object told which level the user picks. Held weakly.
 */
@property(nonatomic, weak, nullable) id<MusicPlaylistLevelSelectorDelegate> delegate;

/**
 * Sets the navigation title, installs a Cancel back button, and builds the level counts.
...
/**
 * Loads the view by chaining up to the superclass.
 * @ghidraAddress 0x1c2e5c
 */
- (void)loadView;

/**
 * Builds @c numArray : the number of charts found at each of the ten levels.
 *
...
/**
 * Decodes a tune archive at @p path into its info dictionary.
 *
