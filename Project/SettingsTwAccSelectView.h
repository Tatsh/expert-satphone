/**
 * @file
 * A modal table listing the Twitter-account accessory items, letting the user pick one to
 * equip or unlock.
 *
...
/**
 * What a @c SettingsTwAccSelectView tells its owner as accessory rows are chosen, changed,
...
/**
 * A modal accessory-select table backed by a caller-supplied array of item rows.
 */
...
/**
 * Builds (or reuses) the accessory cell for a row and shows a checkmark on the equipped one.
 * @param tableView The table posing the question.
 * @param indexPath The row's index path.
