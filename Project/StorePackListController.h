...
/**
 * @brief Receives the pack-list controller's download outcomes.
 *
...
@optional
/**
 * @brief Sent when a catalogue or product fetch fails.
...
/**
 * @brief Sent when a catalogue fetch completes with packs.
 * @param controller The reporting controller.
...
/**
 * @brief Sent when a fetch completes but yields no packs.
 * @param controller The reporting controller.
 */
- (void)packListDownloadNothing:(StorePackListController *)controller;
/**
 * @brief Sent when an additional single-pack fetch resolves its product.
 * @param controller The reporting controller.
...
/**
 * @brief Fetches, groups, and vends the store's genre and pack lists.
 */
@interface StorePackListController : NSObject <DownloaderDelegate, SKProductsRequestDelegate>

/**
 * @brief The App Store country recorded from the most recent product's price locale.
...
/**
 * @brief Builds an empty controller seeded with a single "all" genre.
...
/**
 * @brief The number of genres currently held.
...
/**
 * @brief The display names of the held genres, in order.
...
/**
 * @brief A copy of the held genre models, in order.
...
/**
 * @brief Appends genres from a two-element [ids, names] pair, zipping them into genre models.
...
/**
 * @brief Appends genres from a two-element [ids, names] pair and attaches extend info by ID.
...
/**
 * @brief Builds the promotion list from promotion entries and the valid StoreKit products.
...
/**
 * @brief The most recently built promotion list.
...
/**
 * @brief The list position of the genre with the given identifier.
 *
...
/**
 * @brief The genre model with the given identifier.
...
/**
 * @brief The genre model at the given list position.
...
/**
 * @brief Starts a fresh fetch of the first catalogue page for the given genre identifier.
...
/**
 * @brief Starts fetching the next catalogue page for the genre at the given list position.
...
/**
 * @brief Starts fetching the next page for the given genre model, found by identity.
...
/**
 * @brief Starts a StoreKit request to resolve a single additional pack by identifier.
...
/**
 * @brief Cancels any in-flight download or product request and clears the pending additional pack.
 * @ghidraAddress 0xcd16c
 */
- (void)cancelFetching;

/**
 * @brief Whether a download or product request is currently in flight.
 *
...
/**
 * @brief Whether the genre and promotion lists have been loaded at least once.
 * @ghidraAddress 0xcebdc
 */
@property(nonatomic, readonly) BOOL initiallyLoaded;

/**
 * @brief The pending additional-pack identifier, held for the duration of its StoreKit request.
 * @ghidraAddress 0xceb84 (getter), 0xceb94 (setter)
 */
@property(nonatomic, strong, nullable) NSNumber *additionalPackID;

/**
 * @brief The download-outcome delegate.
