...
/**
 * A store pack's detail screen (version 2).
 */
...
/**
 * Builds the gradient-backed root view, the three tables (tune list, recommended-pack list,
...
/**
 * Refreshes each header's purchase button title, colour, and enabled state, and shows or
...
/**
 * Installs the headers, refreshes the purchase state, starts the header artwork download,
...
/**
 * Extend-button tap: hides the button on every header and forwards extend-download to the
 * delegate.
...
/**
 * Downloader completion: parses the pack detail for the info downloader, loads and plays a
...
/**
 * Relation-tab tap: cross-fades between the tune list and the recommended-pack list, snaps
