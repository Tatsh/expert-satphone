/**
 * @file
 * The song-grid layout for the music-selection collection view.
 */
...
/**
 * The column-type selector: 0 is the three-column grid, 1 the four-column grid, and 2 the
 * five-column grid.
...
/**
 * Returns the horizontal spacing between adjacent cells for the given column type on the
...
/**
 * Returns the vertical spacing between adjacent cell rows for the given column type on the
 * current device idiom.
 * @param columnType The column-type selector.
...
/**
 * Returns the leading horizontal margin for the given column type on the current device
 * idiom.
 * @param columnType The column-type selector.
...
/**
 * Returns the leading vertical margin for the given column type on the current device idiom.
 * @param columnType The column-type selector.
...
/**
 * Records the superclass's target offset for the proposed offset and freezes it, so that the
...
/**
 * Clears the frozen-offset flag so paging resumes deferring to the superclass.
 * @ghidraAddress 0xd9e08
 */
- (void)cancelIgnoreOffset;

/**
 * Returns the cell scale factor for the current column type.
...
/**
 * Caches the device idiom and clears the paging state.
...
/**
 * Tears the layout down.
 * @ghidraAddress 0xd9748
 */
- (void)dealloc;

/**
 * Lays the grid out, filling the cached attributes and the content width.
...
/**
 * The attributes of every cell intersecting a rectangle.
...
/**
 * Whether a bounds change invalidates the layout.
...
/**
 * The content offset to settle on, honouring a frozen offset when one is set.
