...
/**
 * What a @c JcfDownloadView tells its owner.
 *
...
@optional
/**
 * Sent from the Cancel/End button once the download has finished or been dismissed.
...
/**
 * Sent from the move-to-store button after a successful download whose tune belongs to a
 * purchasable pack.
 * @param view The modal (see the reconstruction note: the binary passes the delegate itself here).
...
/**
 * A gradient-backed board that downloads a @c .jcf chart and reports the outcome.
 */
...
/**
 * Builds the board, its label, the three buttons, and the spinner, then starts the
 * download.
 *
...
/**
 * Cancel/End button action: ends the modal through @c -downloadEnd .
...
/**
 * Move-to-store button action: tells the delegate @c -jcfDownloadMoveStore:packID: .
 * @param sender The button.
...
/**
 * Adds the full-board progress label showing "downloading".
 *
...
/**
 * Tells the delegate @c -jcfDownloadEnd: that the modal has finished.
 * @ghidraAddress 0x1ec78c
 */
- (void)downloadEnd;

/**
 * @c JcfDownloaderDelegate : the download failed; shows the message and an OK button.
...
/**
 * @c JcfDownloaderDelegate : the edit-slot cap was reached; shows the cap message and an OK
 * button.
...
/**
 * @c JcfDownloaderDelegate : the tune is not built in and belongs to a purchasable pack;
...
/**
 * @c JcfDownloaderDelegate : the chart downloaded and saved; stores the tune id and shows
...
/**
 * The tune id resolved by a successful download.
