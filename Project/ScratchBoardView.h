...
/**
 * A board of sixteen scratch panels over their backgrounds, plus a count/message view.
 */
@interface ScratchBoardView : UIView

/**
 * The delegate forwarded to every scratch panel and the message view. Held weakly.
 * @ghidraAddress 0x821f8 (getter)
 */
@property(nonatomic, weak, nullable) id aDelegate;

/**
 * Builds the board, its panel grid, and the message view, scaled for the device.
...
/**
 * Sets the delegate on the board and forwards it to every panel.
...
/**
 * Refreshes the message view's scratch count from the challenge status.
 * @ghidraAddress 0x81f58
 */
- (void)refreshScratchCount;

/**
 * Refreshes every panel's enabled state from the challenge status.
 * @ghidraAddress 0x81fcc
 */
- (void)refreshScratchTable;

/**
 * Forwards a timer tick to every panel and the message view.
