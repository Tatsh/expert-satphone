...
/**
 * Maintains a persistent map of search keys to their expanded word lists, merging new
...
/**
 * Copies the bundled seed dictionary into the documents directory, replacing any existing
 * copy.
...
/**
 * Loads the persisted dictionary (or an empty one when none exists).
...
/**
 * A snapshot copy of the current expansion dictionary.
...
/**
 * Merges a word list into the entry for a key, de-duplicating the union.
...
/**
 * Merges every key/word-list pair of another dictionary into this one.
...
/**
 * Loads the persisted dictionary from the documents directory.
 * @ghidraAddress 0x15f1ec
 */
- (void)loadDictionary;

/**
 * Saves the current dictionary as JSON to the documents directory.
