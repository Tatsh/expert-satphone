...
/**
 * Lists downloaded/purchased tunes with a per-row delete or re-download action.
 */
...
/**
 * Builds the view: a full-bounds vertical gradient behind a non-selectable table.
 * @ghidraAddress 0x90f28
 */
- (void)loadView;

/**
 * Vends and lays out a row for a purchased tune.
...
/**
 * Tints alternating rows before display.
 * @param aTableView The table asking.
...
/**
 * The per-row action button's handler: delete an owned tune or start a re-download.
...
/**
 * Builds the download-task queue for the working row and starts the download manager.
 * @ghidraAddress 0x922f4
 */
- (void)startDownloadMusic;

/**
 * Updates the modal dialog's message when a download task starts.
...
/**
 * The tune-info fetch finished: register the tune(s), save the list, and start the pack
 * download.
...
/**
 * The tune-info fetch failed: proceed to the pack download anyway.
...
/**
 * The modal dialog's abort button was pressed: cancel everything and dismiss.
...
/**
 * A confirmation alert's button was tapped; performs the delete when confirmed.
...
/**
 * An alert was dismissed; clears the working row.
...
/**
 * The pack download queue completed: reload the table and dismiss the dialog.
...
/**
 * The pack download queue failed: show an error alert, reload, and dismiss.
...
/**
 * A pack download task made progress: drive the dialog's progress bar.
...
/**
 * Handles a low-memory warning by chaining to super.
 * @ghidraAddress 0x9320c
 */
- (void)didReceiveMemoryWarning;

/**
 * Releases the table when the view is unloaded.
...
/**
 * Chains to super only; the strong ivars are torn down by the generated destructor.
