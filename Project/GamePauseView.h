...
/**
 * What a @c GamePauseView tells its owner when a button is tapped.
 *
...
/** Sent when the resume button is tapped. */
- (void)resumeInPauseView;
/** Sent when the restart button is tapped. */
- (void)restartInPauseView;
/** Sent when the end button is tapped. */
- (void)endInPauseView;
@end

/**
 * A modal pause overlay themed per the current game theme.
 */
@interface GamePauseView : UIView

/**
 * The object told when a button is tapped. Held weakly.
 * @ghidraAddress 0x98a50 (getter)
 */
@property(nonatomic, weak, nullable) id<GamePauseViewDelegate> delegate;

/**
 * Builds the overlay, themed and laid out per the current theme and challenge mode.
...
/**
 * Presents the overlay over a view, optionally fading it in.
