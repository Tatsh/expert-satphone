/**
 * @file
 * The store's recommended-pack table: a self-hosting @c UITableView that lists recommended
...
/**
 * A table of recommended store packs with lazily-downloaded artwork and an infinite-scroll
 * "load more" row.
 *
...
/** The packs to display. */
@property(nonatomic, strong, nullable) NSArray *packList;

/**
 * The owner told when a pack is tapped.
 *
...
/**
 * The pack whose recommendations this table lists.
 *
...
/**
 * Ends the "load more" spinner state and re-enables selection.
...
/**
 * @c StoreRecommendPackViewDelegate callback: opens the tapped tile's pack through
...
/**
 * The number of pack rows: the pack count on iPhone, or half of it (rounded up) on iPad
...
/**
 * Fills one pack view (a @c StorePackCell or a @c StoreRecommendPackView) from the pack at
...
/**
 * The number of rows in a section, which is @c numPackRows.
 * @param tableView The table view.
...
/**
 * Opens the selected pack through @c parentView (iPhone only), or deselects the "load more"
 * row.
 * @param tableView The table view.
 * @param indexPath The row's index path.
...
/**
 * Empties the artwork cache and reloads the table.
 * @ghidraAddress 0x21440c
 */
- (void)reloadData;

/**
 * Scroll callback. The shipped body is empty (a single @c ret ).
