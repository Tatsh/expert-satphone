...
/**
 * What a @c JcfDownloader tells its owner. The delegate is a bare @c id the binary messages
...
@optional
/**
 * The download failed, with an optional user-facing message.
...
/**
 * The chart downloaded and saved for the given tune id.
...
/**
 * The chart could not be saved because the edit-slot cap was reached.
 * @param downloader The downloader reporting the result.
 */
- (void)finishedSequenceOverCap:(nullable JcfDownloader *)downloader;
/**
 * The follow-up pack lookup finished and the tune is already available.
 * @param downloader The downloader reporting the result.
 */
- (void)finishedSequenceDownload:(nullable JcfDownloader *)downloader;
/**
 * The tune is not part of any purchasable pack.
 * @param downloader The downloader reporting the result.
...
/**
 * Downloads a shared edit chart by its custom id.
 */
@interface JcfDownloader : NSObject <EditorIDManagerDelegate>

/**
 * The store-new-info endpoint used for the comprised-pack lookup.
...
/**
 * Begins downloading the chart for a custom id, minting an editor id first if needed.
...
/**
 * Starts the jubeatLab sequence-download API for the custom id.
 * @ghidraAddress 0x1d474c
 */
- (void)downloadStart;

/**
 * jubeatLab access progress callback (a no-op).
...
/**
 * jubeatLab access failure callback: reports the error for the matching request.
...
/**
 * Relays a plain download failure to the delegate and drops the sequence downloader.
 * @ghidraAddress 0x1d4948
 */
- (void)downloadFailedDelegate;

/**
 * Whether a built-in @c .jbt chart already exists for a tune id.
...
/**
 * jubeatLab access success callback: decodes and saves the chart, then looks up its pack.
...
/**
 * @c EditorIDManager success callback: begins the download once an editor id exists.
...
/**
 * @c EditorIDManager failure callback: reports the download error.
