...
/**
 * A view controller presenting the jubeatLab editor page in a @c UIWebView.
 */
// clang-format off
...
/**
 * Builds the request for the editor page: an @c NSURLRequest wrapping
...
/**
 * Loads the stored next request into the web view.
 * @ghidraAddress 0x1f7af0
 */
- (void)openStartPage;

/**
 * Opens a jubeatLab session: builds a @c jubeatLabAccess session client bound to this
...
/**
 * Initialises the controller: builds the web view and its editor-page request, then either
...
/**
 * Gates the page's navigation. A request to the session-failed URL re-opens a session; a
...
/**
 * Shows a communication-error alert on a load failure.
...
/**
 * jubeatLab callback sent as the session proceeds. Does nothing.
...
/**
 * jubeatLab callback sent when the session fails: clears the session and shows a
...
/**
 * jubeatLab callback sent when the session finishes. On a keychain-switch status it replaces
...
/**
 * @c EditorIDManager callback: editor identifier provisioning succeeded. Opens a session, or
...
/**
 * @c EditorIDManager callback: editor identifier provisioning failed. Shows the server
...
/**
 * Presents the social share composer for a service type, seeded with the stored share text.
