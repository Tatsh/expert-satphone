...
/**
 * Whether the inner web view is allowed to scroll (bounce) when dragged.
 */
...
/**
 * Apply the shared container configuration (transparent, non-opaque, fully flexible
...
/**
 * Remove the loading indicator and inner web view, fire the sound-use-finish notice, notify
...
/**
 * Notify the delegate that the advert has begun using sound, so the host can pause its own
 * audio.
...
/**
 * Notify the delegate that the advert has finished using sound, so the host can resume its
 * own audio.
