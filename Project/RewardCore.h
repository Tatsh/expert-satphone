...
/**
 * Clear the initialisation flag, forget the stored campaign flag, and drop the reward
...
/**
 * Start the session (via @c startWithCallback:), refreshing the reward auth session after.
 * @param block The completion block invoked with an error, or @c nil on success.
...
/**
 * Query the full list of advertised application identifiers.
...
/**
 * Present a reward-video view for @p query through @c ApplilinkViewManager, hosted on the
