/**
 * @file
 * A horizontally-scrolling store genre picker: a row of tappable genre banners inside a
...
/**
 * What a @c StoreGenreSelectView tells its owner when the selection changes.
 */
@protocol StoreGenreSelectViewDelegate <NSObject>

/**
 * Sent when a genre banner is tapped, carrying the genre index within the genre list.
...
/**
 * A paging, wrap-around row of genre banners with a delegate-reported selection.
 */
@interface StoreGenreSelectView : UIView <UIScrollViewDelegate>

/**
 * The object told when the selected genre changes.
 */
@property(nonatomic, weak, nullable) id<StoreGenreSelectViewDelegate> delegate;

/**
 * Builds the scroll view, one banner per genre plus wrap-around copies, and selects the
 * first genre.
...
/**
 * Scroll delegate hook invoked as dragging begins. Inert in the shipped binary.
...
/**
 * Keeps the paging offset inside the real banners by looping it when it drifts into a
 * wrap-around pad.
 * @param scrollView The scrolling view.
...
/**
 * Scroll delegate hook invoked when dragging ends. Inert in the shipped binary.
 * @param scrollView The scrolling view.
...
/**
 * Scroll delegate hook invoked as deceleration begins. Inert in the shipped binary.
 * @param scrollView The scrolling view.
...
/**
 * Scroll delegate hook invoked when deceleration ends. Inert in the shipped binary.
 * @param scrollView The scrolling view.
...
/**
 * Banner tap handler: selects the tapped genre and notifies the delegate.
...
/**
 * Selects a genre by index, highlighting its banner and animating the scroll to it.
...
/**
 * Marks the banner for the given genre index as selected and every other as not.
