/**
 * @file
 * One challenge mission sheet: a named, dated group of missions with its reward and
 * progress.
 *
...
/**
 * A challenge mission sheet's identity, dates, reward, and mission tables.
 */
@interface ChallengeMissionSheet : NSObject

/** Whether the player has confirmed (seen) the sheet. Wire key @c "confirm_flag". */
@property(nonatomic, readonly) BOOL bConfirmed;
/** The sheet's display name. Wire key @c "name". */
@property(nonatomic, readonly, nullable) NSString *sheetName;
/** The sheet's identifier. Wire key @c "sheet_id". */
@property(nonatomic, readonly) int sheetID;
/** The number of missions in the sheet. Wire key @c "amount". */
@property(nonatomic, readonly) int missionCnt;
/** The total mission count across the campaign. Wire key @c "total". */
@property(nonatomic, readonly) int totalMissionCnt;
/** Whether this is an event sheet. Wire key @c "is_event". */
@property(nonatomic, readonly) BOOL isEvent;
/** The sheet's version tag. Wire key @c "version". Defaults to the empty string. */
@property(nonatomic, readonly, nullable) NSString *updateTime;
/** When the sheet becomes visible. Wire key @c "view_start". Defaults to the empty string.
 */
@property(nonatomic, readonly, nullable) NSString *sheetStartDate;
/** When the sheet stops being visible. Wire key @c "view_end". Defaults to empty string. */
@property(nonatomic, readonly, nullable) NSString *sheetEndDate;
/** The sheet's background image URL. */
@property(nonatomic, readonly, nullable) NSString *sheetBgURL;
/** The sheet's reward. Wire key @c "reward". */
@property(nonatomic, readonly, nullable) ChallengeMissionReward *reward;
/** The @c ChallengeMissionTerms for each mission. Built from wire key @c "mission_list". */
@property(nonatomic, readonly, nullable) NSArray *missionTable;
/** The @c ChallengeMissionAchieve for each mission, when every mission has one. */
@property(nonatomic, readonly, nullable) NSArray *missionAchieveTable;
/** The @c ChallengeMissionPlayTerm for each mission. */
@property(nonatomic, readonly, nullable) NSArray *missionSimpleTable;
/** Whether the sheet holds a complete, saveable mission list. */
...
/**
 * Fills the sheet in from a server dictionary.
...
/**
 * Deciphers and unarchives a saved sheet, filling the object in when its id matches.
...
/**
 * Rebuilds the mission-terms table (and the reward) from a server dictionary.
 *
...
/**
 * Rebuilds the achievement and play-term tables from an achievement dictionary.
 *
...
/**
 * Ciphers an archived copy of the sheet for local persistence.
...
/**
 * Writes the sheet back out in the server's dictionary format.
...
/**
 * Builds the @c "mission_list" dictionary mapping each mission's dictionary to its boxed id.
...
/**
 * Marks the sheet confirmed, reporting whether this call was the one that changed it.
