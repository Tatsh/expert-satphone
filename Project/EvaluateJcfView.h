...
/**
 * What an @c EvaluateJcfView tells its owner. The delegate is a bare weak @c id the binary
...
@optional
/**
 * The evaluation panel finished and should be dismissed.
...
/**
 * The panel that lets the player rate a downloaded custom chart.
 */
@interface EvaluateJcfView : UIView

/**
 * The layer class used to back the view.
...
/**
 * Builds and configures a stray @c StoreButton .
 *
...
/**
 * Builds the evaluation panel for a downloaded chart.
...
/**
 * Cancel-button handler: closes the panel.
...
/**
 * Good-job-button handler: POSTs a good-job vote and dims the button.
...
/**
 * Level-commit-button handler: POSTs the chosen level and disables the button.
...
/**
 * Slider handler: snaps the slider to the nearest whole level and applies it.
...
/**
 * Applies a chosen level: records it, moves the slider, and updates the title.
...
/**
 * Reports the panel closing to the delegate.
 * @ghidraAddress 0x1fbd34
 */
- (void)evaluateEnd;

/**
 * jubeatLab access failure callback: drops the matching request.
...
/**
 * jubeatLab access success callback: records the vote in the editor info and closes.
