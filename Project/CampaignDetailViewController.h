/**
 * @file
 * The campaign-detail page: a table with a campaign header, the item's terms and licence,
...
/**
 * Receives the campaign-detail page's lifecycle callbacks.
 *
...
/**
 * The page's delegate. Held weakly.
...
/**
 * The store campaign controller that owns this page. Held weakly.
...
/**
 * Builds the gradient-backed root view, the pack-background art, the detail table, the
...
/**
 * Stores the campaign item to display.
...
/**
 * Clears the stored campaign item.
 * @ghidraAddress 0x1e9444
 */
- (void)removeCampaignInfo;

/**
 * Replaces the campaign item and forwards it to the header.
...
/**
 * Swaps in a fresh campaign item: cancels the artwork download, stops the sample, reloads
...
/**
 * Navigates to the pack info. The shipped body is empty.
 * @ghidraAddress 0x1e9564
 */
- (void)showPackInfo;

/**
 * Populates the header and table from the current item, downloads the artwork, and sizes
...
/**
 * Header download-button tap: forwards to the owning controller's item download.
...
/**
 * Header link-button tap: forwards to the owning controller's external link.
...
/**
 * SKStoreProductViewController finish: dismisses it, then clears the reference once the
...
/**
 * Cancels the artwork download and detaches its delegate.
 * @ghidraAddress 0x1e9dbc
 */
- (void)stopDownloadArtworks;

/**
 * Tears down the view references and cancels the artwork download.
 * @ghidraAddress 0x1e9d30
 */
- (void)removeItem;

/**
 * Sends @c -detailClose to every cell that answers it, so the table can shut its rows.
