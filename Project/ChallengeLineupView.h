...
/**
 * Told when the line-up modal is closed or a purchasable pack is chosen.
 *
...
/** The close button was tapped and the fade-out has finished. */
- (void)closeLineupView;
/**
 * Open the jubeat store on the pack containing the chosen tune.
...
/**
 * A dimmed-overlay modal listing the current scratch line-up over a background plate with a
...
/**
 * The delegate told about close and store events. Held weakly.
 * @ghidraAddress 0x14765c (getter), 0x14767c (setter)
 */
@property(nonatomic, weak, nullable) id<ChallengeLineupViewDelegate> aDelegate;

/**
 * Builds the modal over the given frame, initially hidden. The rows come from
