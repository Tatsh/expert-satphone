...
/**
 * Told when the rival-search modal wants to close itself.
 */
...
/**
 * A modal that takes a rival's search ID in a text field, looks the rival up through the
 * server, and — once found — shows an add/cancel prompt that registers the rival.
 */
@interface ChallengeRivalSearchView : UIView

/**
 * The delegate told when the modal should close. Held weakly.
...
/**
 * Builds the search box and the (initially hidden) rival-add prompt.
