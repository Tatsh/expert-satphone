/**
 * @file
 * The header panel atop a campaign-detail page.
 *
...
/**
 * Receives the header's button taps.
 *
...
/**
 * Sent when the download button is tapped.
 * @param sender The download button.
 */
- (void)doPurchase:(nullable id)sender;

/**
 * Sent when the link button is tapped.
...
/**
 * A campaign item's detail header: artwork with its reflection, name, comment, a download
 * button, a link button, and a tappable BGM sample control.
 */
@interface CampaignDetailHeaderView : UIView <DownloaderDelegate>

/**
 * The object that carries out the download and link button taps. Held weakly.
...
/**
 * Lays the header out for a campaign item: sizes the name and comment to their text,
...
/**
 * Replaces the campaign item and refreshes the download button.
...
/**
 * Toggles BGM sample playback: starts a sample download and spins the indicator, or stops
...
/**
 * Fades out any BGM sample, tears the sample download down, and resets the control.
 * @ghidraAddress 0x123240
 */
- (void)stopSample;

/**
 * Handles the audio player finishing the BGM sample: clears the playing state and hides the
 * playing marker.
