...
/**
 * What a @c JcfUploader tells its owner. The delegate is a bare @c id the binary messages
 * dynamically behind @c -respondsToSelector: ; this protocol only documents the selectors.
 */
@protocol JcfUploaderDelegate <NSObject>
@optional
/**
 * The upload succeeded, with the server's returned info.
...
/**
 * The upload was rejected for NG words (title, editor, and comment).
...
/**
 * The upload failed, with an optional user-facing message.
...
/**
 * Uploads an edit chart to jubeatLab.
 */
@interface JcfUploader : NSObject <EditorIDManagerDelegate>

/**
 * Reads a keychain generic-password value for an account key.
...
/**
 * Prepares an upload of the given chart data.
...
/**
 * Begins the upload, minting an editor id first if needed.
 * @ghidraAddress 0x1d8184
 */
- (void)start;

/**
 * Starts the jubeatLab sequence-upload API for the stored chart data.
 * @ghidraAddress 0x1d8214
 */
- (void)uploadStart;

/**
 * Relays an upload error to the delegate.
...
/**
 * jubeatLab access success callback: maps the response status to the delegate callbacks.
...
/**
 * @c EditorIDManager success callback: begins the upload once an editor id exists.
...
/**
 * @c EditorIDManager failure callback: reports the upload error.
