...
/**
 * Told when the name field's edit is committed.
 */
@protocol ChallengeTextInputViewDelegate <NSObject>
@optional
/**
 * The field's text was committed.
...
/**
 * A single-line name-entry field paired with a commit button, clamping the entry to at most
...
/** The name-entry text field. @ghidraAddress 0x94888 (getter) */
@property(nonatomic, strong, nullable) UITextField *nameBox;
/** The commit button. @ghidraAddress 0x948ac (getter) */
@property(nonatomic, strong, nullable) UIButton *changeBtn;
/** The delegate told when the entry is committed. Held weakly.
 *  @ghidraAddress 0x948d0 (getter), 0x948f0 (setter) */
@property(nonatomic, weak, nullable) id<ChallengeTextInputViewDelegate> aDelegate;
/** The last backed-up field text, clamped to 20 characters. @ghidraAddress 0x94904 (getter)
 */
@property(nonatomic, readonly, nullable) NSString *inputText;

/**
 * Clears the field text, the backed-up text, and the button image.
 * @ghidraAddress 0x9415c
 */
- (void)resetView;

/**
 * Sets the field's text and backs it up as the current input.
...
/**
 * Whether a string round-trips to empty through Shift-JIS, meaning it is purely
 * pictographic.
