...
/**
 * Receives the carousel's banner-tap selections.
 *
...
/**
 * A pack promotion's banner was tapped.
...
/**
 * A genre promotion's banner was tapped.
 * @param view The carousel that sent the message.
...
/**
 * A paging carousel of promotion banners with auto-advance and per-banner sample playback.
 */
@interface StorePromotionView : UIView <UIScrollViewDelegate>

/**
 * The tap-selection delegate.
 * @ghidraAddress 0x1bd5d0 (getter), 0x1bd5f0 (setter)
 */
@property(nonatomic, weak, nullable) id<StorePromotionViewDelegate> delegate;

/**
 * Builds the carousel from a list of promotions and lays out every banner.
...
/**
 * Updates the sample button's image and the track label's opacity for the current mode.
 * @ghidraAddress 0x1bc204
 */
- (void)setSampleButtonImage;

/**
 * Redirects a touch that lands on the view itself down to the scroll view.
...
/**
 * Arms the auto-advance timer and plays the centred banner's thumbnail.
 * @ghidraAddress 0x1bc314
 */
- (void)beginLoop;

/**
 * Starts fetching every banner's artwork, then begins the auto-advance loop.
 * @ghidraAddress 0x1bc418
 */
- (void)start;

/**
 * Pauses the carousel: stops the timer and the thumbnail playback.
 * @ghidraAddress 0x1bc55c
 */
- (void)pause;

/**
 * Resumes a paused carousel.
 * @ghidraAddress 0x1bc600
 */
- (void)resume;

/**
 * Stops the carousel: invalidates the timer and mutes the thumbnail.
 * @ghidraAddress 0x1bc624
 */
- (void)stop;

/**
 * Scrolls to the next banner and plays its thumbnail. The timer's action.
 * @ghidraAddress 0x1bc690
 */
- (void)nextBanner;

/**
 * Handles a banner tap, reporting the pack or genre to the delegate.
...
/**
 * Toggles the sample-play mode, persists it, and refreshes the button.
...
/**
 * Cancels the auto-advance timer while the user drags.
...
/**
 * Teleports the offset across the wrap-around seam to fake an infinite strip.
 * @param scrollView The scroll view.
...
/**
 * Re-arms the auto-advance timer once a decelerating scroll settles.
 * @param scrollView The scroll view.
...
/**
 * Plays the thumbnail of the banner at @p index, updating the track label.
...
/**
 * Enables or disables thumbnail playback, playing or muting the centred banner.
...
/**
 * Returns a promotion's cached thumbnail data, enqueuing a download on a miss.
...
/**
 * Empties the thumbnail cache and cancels all pending downloads.
 * @ghidraAddress 0x1bcf54
 */
- (void)clearThumbnailCache;

/**
 * Downloads one thumbnail on the operation queue and caches its bytes.
...
/**
 * Loads and plays a thumbnail's sound clip through the audio manager.
...
/**
 * Fades out and clears the currently-playing thumbnail clip.
