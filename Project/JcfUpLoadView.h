/**
 * @file
 * The modal board that uploads one jubeatLab custom sequence (a @c .jcf chart).
 *
...
/**
 * What a @c JcfUpLoadView tells its owner.
 *
...
@optional
/**
 * Sent from the Cancel/End button once the upload has finished or been dismissed.
...
/**
 * A gradient-backed board that uploads a @c .jcf chart and shares the outcome.
 */
@interface JcfUpLoadView : UIView <JcfUploaderDelegate>

/**
 * The layer class backing the board: a @c CAGradientLayer .
...
/**
 * Builds the board, its label, the confirmation buttons, and the spinner, then arms the
 * owned @c JcfUploader .
 *
...
/**
 * Cancel/End button action: ends the modal through @c -uploadEnd .
...
/**
 * Upload-OK button action: shows the spinner, swaps the confirmation buttons for the single
...
/**
 * Presents the licence-agreement overlay, whose callbacks gate the upload.
 * @ghidraAddress 0x1f4468
 */
- (void)licenseStart;

/**
 * Updates the label to the in-progress text and starts the owned @c JcfUploader .
 * @ghidraAddress 0x1f44d8
 */
- (void)uploadStart;

/**
 * Unused entry point (an empty body in the binary).
 * @ghidraAddress 0x1f4528
 */
- (void)startUpload;

/**
 * Tells the delegate @c -uploadEnd: that the modal has finished.
 * @ghidraAddress 0x1f452c
 */
- (void)uploadEnd;

/**
 * @c JcfUploaderDelegate : the upload failed; shows the message and an OK button.
...
/**
 * @c JcfUploaderDelegate : the upload was rejected for NG words; shows the rejection message
...
/**
 * @c JcfUploaderDelegate : the upload succeeded; stores the sequence id and share text,
...
/**
 * @c LicenseAgreementView callback: the agreement step failed; shows the message, an OK
...
/**
 * @c LicenseAgreementView callback: the agreement was accepted; starts the upload and
 * removes the overlay.
 * @param sender The licence view.
...
/**
 * @c LicenseAgreementView callback: the agreement was declined; ends the modal.
 * @param sender The licence view.
...
/**
 * Presents the share composer for the given social service.
...
/**
 * Twitter share button action: posts through @c -socialSend: with the Twitter service.
 * @param sender The button.
...
/**
 * Facebook share button action: posts through @c -socialSend: with the Facebook service,
