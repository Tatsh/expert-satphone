...
/**
 * What a @c SearchPackIDView tells its owner once the pack lookup settles.
 *
...
@optional
/**
 * Sent when the lookup resolved a pack (or a social recommend string).
 * @param view The modal.
 */
- (void)packIDSearchEnd:(nullable SearchPackIDView *)view;
/**
 * Sent when the lookup completed but no pack contains the tune.
 * @param view The modal.
 */
- (void)packIDSearchFailed:(nullable SearchPackIDView *)view;
/**
 * Sent from the Cancel/Close button, whether the lookup is still running or has settled.
 * @param view The modal.
 */
...
/**
 * A gradient-backed board that resolves the pack owning a tune and reports the outcome.
 */
...
/**
 * A @c StoreButton factory helper.
 *
...
/**
 * Whether a music id is a built-in bundle tune.
 *
...
/**
 * Builds the modal for the tune carried by a music object and starts the pack lookup.
 *
...
/**
 * Builds the board, its label, the three buttons, and the spinner, then posts the lookup.
 *
...
/**
 * Cancel/Close button action: cancels the lookup and tells the delegate
...
/**
 * Overlays the full-board progress label and starts the pack-lookup download.
 * @ghidraAddress 0x1158f0
 */
- (void)startDownload;

/**
 * @c DownloaderDelegate : the lookup failed; shows the failure message.
...
/**
 * @c DownloaderDelegate : the lookup completed; parses the pack id and message, then reports
...
/**
 * Shows the download-failed message and retitles the button Close.
 * @ghidraAddress 0x115cd4
 */
- (void)searchPackFailed;

/**
 * Shows the no-pack message, retitles the button Close, and tells the delegate
...
/**
 * The pack id resolved by the lookup.
...
/**
 * The recommend/message string the lookup returned.
