...
/**
 * A status bar showing the play-coin count over its limit, the J-cube count, the coin
 * regeneration time, and a button to buy cubes.
 */
@interface ChallengeStatusView : UIView

/**
 * The delegate told when the buy-cube button (or its confirmation) fires. Held weakly.
 * @ghidraAddress 0x86ee4 (getter)
 */
@property(nonatomic, weak, nullable) id aDelegate;

/**
 * Builds the status bar: the digit slots, slash, rest-time label, and buy-cube button.
...
/**
 * Refreshes every displayed number and the coin rest-time label from the challenge status.
 * @ghidraAddress 0x868bc
 */
- (void)updateDisplayStatus;

/**
 * The per-tick refresh: regenerates a coin when its timer elapses and updates the display.
