...
/**
 * A rounded plate that carries one @c StoreLeafletCell and forwards its open request on to
 * an owner.
 *
...
/**
 * The object told to open a pack's detail screen.
 *
...
/**
 * Builds the plate and its single leaflet cell.
...
/**
 * The number of pack rows the table would show. Always @c 0 as shipped.
 * @return Zero.
...
/**
 * Would vend a row cell. Unreachable because @c numPackRows is @c 0.
...
/**
 * Scroll callback. The shipped body is empty (a single @c ret ), so the view ignores
 * scrolling.
...
/**
 * Asks @c aDelegate to open a pack's detail screen, if it responds.
