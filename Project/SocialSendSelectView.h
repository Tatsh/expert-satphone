/**
 * @file
 * A modal "select where to send" panel: a dimmed, rounded gradient board carrying a title,
...
/**
 * What a @c SocialSendSelectView tells its owner when a target is chosen or the panel is
 * dismissed.
 *
...
@optional
/**
 * Sent when OK is pressed, carrying the chosen social service type and the message text.
...
/**
 * Sent when Cancel is pressed, carrying the panel itself.
...
/**
 * A modal social-target picker over a dimmed gradient board.
 */
...
/**
 * Builds a discardable store-style button.
 *
...
/**
 * Builds the whole modal: the gradient board, its title, the target table with its inner
...
/**
 * OK button action: forwards the chosen social type and message to the delegate.
...
/**
 * Cancel button action: tells the delegate the panel was cancelled.
