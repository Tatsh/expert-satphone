...
/**
 * Stores the list of scratch-panel resource descriptors and each panel's enciphered image
...
/**
 * The resource descriptors, each a dictionary keyed by @c item_id .
 * @ghidraAddress 0x143b00 (getter), 0x143b10 (setter)
 */
@property(nonatomic, strong, nullable) NSMutableArray *arrayResource;

/**
 * Loads and deciphers the resource list from disk (or starts an empty one).
 * @ghidraAddress 0x142c10
 */
- (void)loadResourceList;

/**
 * Enciphers and saves the resource list to disk with a random-nonce header.
 * @ghidraAddress 0x142f30
 */
- (void)saveResourceList;

/**
 * Adds or replaces a panel resource descriptor, keyed by its item id.
...
/**
 * Loads and deciphers a panel's image data, validating its item-id header.
...
/**
 * Enciphers and saves a panel's image data with an item-id header.
...
/**
 * Deletes a panel's stored image data.
...
/**
 * Marks a file URL as excluded from iCloud backup.
