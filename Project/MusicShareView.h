/**
 * @file
 * A modal "share music" panel over a dimmed, rounded gradient board: it lists the nearby
...
/**
 * A modal peer-host picker for sharing music, laid over a dimmed gradient board.
 */
@interface MusicShareView : UIView <UITableViewDataSource, UITableViewDelegate>

/**
 * The owning controller, told which host to connect to and when the panel is cancelled.
 *
...
/**
 * Backs the view with a @c CAGradientLayer .
...
/**
 * Builds the whole modal: the gradient board, its host icon and join-background images, the
...
/**
 * Cancel button action: tells the controller to cancel the share.
...
/**
 * Enters the searching state: shows "Searching hosts", makes the host table selectable, and
...
/**
 * Enters the connecting state: locks the host table against selection and shows
 * "Connecting".
...
/**
 * Enters the connected state: detaches the host table as data source and delegate, fades it
...
/**
 * Adds a discovered host, taking its display name from the peer itself, and reloads the
...
/**
 * Removes a host row, resetting the message to "Searching hosts" once the list empties.
 *
...
/**
 * Adds a discovered host under an explicit display name and reloads the table with an
 * inserted row.
 * @param host The peer that was found.
