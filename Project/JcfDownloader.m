...
- (void)jubeatLabAccessFinished:(jubeatLabAccess *)access {
    if (seqDownloader == access) {
        [self handleSequenceDownloadFinished:access];
    }
    if (packInfoDownloader == access) {
        [self handlePackInfoFinished:access];
    }
}
...
- (void)handleSequenceDownloadFinished:(jubeatLabAccess *)access {
    ...
    if (!alreadyBuiltIn) {
        ...
        packInfoDownloader = [[jubeatLabAccess alloc] initComprisedPackApi:self tuneID:tuneID];
        [packInfoDownloader startAccess];
        return;
    }
    ...
    } else if ([delegate respondsToSelector:@selector(finishedSequenceOverCap:)]) {
        [delegate performSelector:@selector(finishedSequenceOverCap:) withObject:self];
    }
}

// The comprised-pack arm of -jubeatLabAccessFinished:. Reports whether the tune's pack exists.
- (void)handlePackInfoFinished:(jubeatLabAccess *)access {
    NSDictionary *response = [StoreUtil checkStoreResponse:access.getData];
    if (response == nil) {
        if ([delegate respondsToSelector:@selector(finishedSequenceDownload:)]) {
            [delegate performSelector:@selector(finishedSequenceDownload:) withObject:self];
        }
    } else {
        packID = [response[kPackIDKey] intValue];
        NSString *packIDString = [NSString stringWithFormat:kTuneIDFormat, packID];
        if ([delegate respondsToSelector:@selector(finishedSequenceNotExistPack:packID:)]) {
            [delegate performSelector:@selector(finishedSequenceNotExistPack:packID:)
                           withObject:self
