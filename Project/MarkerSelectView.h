...
/**
 * The object notified when the selected marker changes.
 */
@protocol MarkerSelectViewDelegate <NSObject>
@optional
/**
 * Sent after the user picks a different marker.
...
/**
 * The marker selection screen: a paged banner list with a live marker preview.
 */
@interface MarkerSelectView : UIView <UIScrollViewDelegate>

/**
 * Initialises the view, its background, preview, banner scroll view, marker buttons, and
 *        loading overlay.
...
/**
 * Builds the per-theme preview sound name for @p name.
...
/**
 * The background image of the button for the currently persisted marker.
...
/**
 * The @c CADisplayLink callback: advances the preview and hides the press button once its
 *        load has finished.
...
/**
 * Creates the display link targeting @c -loop: , adds it to the run loop, and resets the
 *        preview.
...
/**
 * Pauses the preview and the display link.
 * @ghidraAddress 0x9a19c
 */
- (void)pauseAnimation;

/**
 * Resumes the preview and the display link.
 * @ghidraAddress 0x9a230
 */
- (void)resumeAnimation;

/**
 * Invalidates and releases the display link.
 * @ghidraAddress 0x9a2c0
 */
- (void)stopAnimation;

/**
 * Shows the loading overlay and asynchronously loads the current marker's textures.
...
/**
 * Marker button action: selects the tapped marker, plays a sound, persists it, notifies the
...
/**
 * Stops the preview and releases the preview's textures.
 * @ghidraAddress 0x9aca4
 */
- (void)close;

/**
 * Scroll delegate stub called when dragging begins.
...
/**
 * Wraps the paged scroll offset into the middle copy of the tiled content and re-centres the
 *        banner buttons.
 * @param scrollView The banner scroll view.
...
/**
 * Scroll delegate stub called when deceleration ends.
 * @param scrollView The banner scroll view.
...
/**
 * Scroll delegate stub called when dragging ends.
 * @param scrollView The banner scroll view.
