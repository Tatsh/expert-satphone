...
/**
 * The music-selection detail card shown when a tune is picked.
 *
...
/**
 * Designated initialiser; builds every subview at the origin and hides the modal chrome.
...
/**
 * Stores the base-chart tune; the score argument is ignored.
...
/**
 * Stores the extend-chart tune; the score argument is ignored.
...
/**
 * Opens the tune's iTunes URL in the browser, when it has one.
...
/**
 * Starts the recommend flow: picks the social network from the sender's tag, dims the card,
...
/**
 * Pack-search delegate: the search resolved; fades the sheet out and posts the share text.
...
/**
 * Pack-search delegate: the search was cancelled; fades the sheet and scrim out.
 * @param view The pack-search view.
...
/**
 * The recommend string from the pack-search view, or @c nil when none is installed.
...
/**
 * Removes the pack-search view and the dimming cover, then releases both.
 * @ghidraAddress 0x1286c4
 */
- (void)searchViewDealloc;

/**
 * Presents an @c SLComposeViewController for the given service, seeded with the text.
...
/**
 * Presents the Twitter composer with the given text.
...
/**
 * Resets every base and extend score to -1, clears full-combo flags, and drops music bars.
 * @ghidraAddress 0x128948
 */
- (void)resetScore;

/**
 * Populates the base-chart score, full-combo flags, and music bars from a score record.
...
/**
 * Populates the extend-chart score, full-combo flags, and music bars from a score record.
 * @param score The score record, or @c nil.
...
/**
 * Dismisses the edit popover or jcf download page and closes the detail view.
 * @ghidraAddress 0x12946c
 */
- (void)closePopWindow;

/**
 * Asks the controller to close the detail view.
 * @ghidraAddress 0x129580
 */
- (void)close;

/**
 * Builds the share dictionary by decrypting the tune's packed assets over its base dict.
...
/**
 * Loads the tune's last edit file into the shared edit data manager, or clears it.
 * @ghidraAddress 0x129a84
 */
- (void)loadEditFile;

/**
 * Saves the shared edit data back to the tune's last edit file, when one exists.
 * @ghidraAddress 0x129b50
 */
- (void)saveEditFile;

/**
 * Edit-modal delegate: forwards to @c -saveEditFile.
 * @ghidraAddress 0x129c08
 */
- (void)editModalViewDelegateSaveEditFile;

/**
 * Whether the tune's info may be changed (no host share, a local last-edit file, iPad).
...
/**
 * Whether the tune may be uploaded (as @c -checkEnableInfoChange plus a non-empty note
 * count).
...
/**
 * Whether the tune's chart may be edited (no host share, a local last-edit file, iPad).
...
/**
 * Jcf download-end delegate: on non-iPad, reloads the manage list from the tune's files.
...
/**
 * Dismisses the presented controller and turns the parent to the pack-purchase store page.
...
/**
 * Double-tap gesture on the artwork: asks the controller to select a random tune.
...
/**
 * Long-press gesture on the artwork: toggles the app's random flag on the began state.
...
/**
 * The on-screen position for a difficulty; always @c CGPointZero in this build.
...
/** The base-chart tune. @ghidraAddress 0x12a32c (getter), 0x12a33c (setter) */
@property(nonatomic, strong, nullable) TuneInfo *info;
/** The full-card dimming cover. @ghidraAddress 0x12a350 (getter), 0x12a360 (setter) */
@property(nonatomic, strong, nullable) UIView *coverView;
/** The artwork image. @ghidraAddress 0x12a374 (getter), 0x12a384 (setter) */
@property(nonatomic, strong, nullable) UIImageView *artworkView;
/** The mirrored-artwork reflection. @ghidraAddress 0x12a398 (getter), 0x12a3a8 (setter) */
@property(nonatomic, strong, nullable) UIImageView *reflectionArtworkView;
/** The tune-name plate. @ghidraAddress 0x12a3bc (getter), 0x12a3cc (setter) */
@property(nonatomic, strong, nullable) UIImageView *tuneNameView;
/** The start-play button. @ghidraAddress 0x12a3e0 (getter), 0x12a3f0 (setter) */
@property(nonatomic, strong, nullable) UIButton *buttonStartPlay;
/** The host-share-play button. @ghidraAddress 0x12a404 (getter), 0x12a414 (setter) */
@property(nonatomic, strong, nullable) UIButton *buttonHostSharePlay;
/** The share-status message label. @ghidraAddress 0x12a428 (getter), 0x12a438 (setter) */
@property(nonatomic, strong, nullable) UILabel *labelShareMessage;
/** The share-data progress bar. @ghidraAddress 0x12a44c (getter), 0x12a45c (setter) */
@property(nonatomic, strong, nullable) UIProgressView *shareDataProgress;
/** The link-to-store button. @ghidraAddress 0x12a470 (getter), 0x12a480 (setter) */
@property(nonatomic, strong, nullable) UIButton *buttonLink;
/** The random-select marker. @ghidraAddress 0x12a494 (getter), 0x12a4a4 (setter) */
@property(nonatomic, strong, nullable) UIImageView *randView;
/** The Twitter recommend button. @ghidraAddress 0x12a4b8 (getter), 0x12a4c8 (setter) */
@property(nonatomic, strong, nullable) UIButton *btnRecommendTwitter;
/** The Facebook recommend button. @ghidraAddress 0x12a4dc (getter), 0x12a4ec (setter) */
@property(nonatomic, strong, nullable) UIButton *btnRecommendFacebook;
/** Whether the card is in host-share mode. @ghidraAddress 0x12a500 (getter), 0x12a510
 * (setter) */
@property(nonatomic, assign) BOOL isShared;
/** The edit-file list scroll view. @ghidraAddress 0x12a520 (getter), 0x12a530 (setter) */
@property(nonatomic, strong, nullable) UIScrollView *scrollView;
/** The edit-file delete list controller. @ghidraAddress 0x12a544 (getter), 0x12a554 (setter)
 */
@property(nonatomic, strong, nullable) EditFileListViewDeleteController *pFileListView;
/** The edit-file list popover. @ghidraAddress 0x12a568 (getter), 0x12a578 (setter) */
@property(nonatomic, strong, nullable) UIPopoverController *pLoadList;
/** Whether this is the first selection. @ghidraAddress 0x12a58c (getter), 0x12a59c (setter)
 */
@property(nonatomic, assign) BOOL isFirstSelect;
/** The edit modal. @ghidraAddress 0x12a5ac (getter), 0x12a5bc (setter) */
@property(nonatomic, strong, nullable) EditModalView *pEditModalView;
/** The current edit page index. @ghidraAddress 0x12a5d0 (getter), 0x12a5e0 (setter) */
@property(nonatomic, assign) int editPage;
/** The jcf download navigation controller. @ghidraAddress 0x12a5f0 (getter), 0x12a600
 * (setter) */
@property(nonatomic, strong, nullable) JcfDownloadPageNavController *jcfDownloadPage;
/** The jcf manage navigation controller. @ghidraAddress 0x12a614 (getter), 0x12a624 (setter)
 */
@property(nonatomic, strong, nullable) JcfManageNavController *jcfMan;
/** The share index string. @ghidraAddress 0x12a638 (getter), 0x12a648 (setter) */
@property(nonatomic, strong, nullable) NSString *customIndex;
/** Whether the edit-info modal is open. @ghidraAddress 0x12a65c (getter), 0x12a66c (setter)
 */
@property(nonatomic, assign) BOOL isEditInfoOpen;
/** The base-chart basic level. @ghidraAddress 0x12a67c (getter), 0x12a68c (setter) */
@property(nonatomic, assign) char levelBas;
/** The base-chart advanced level. @ghidraAddress 0x12a69c (getter), 0x12a6ac (setter) */
@property(nonatomic, assign) char levelAdv;
/** The base-chart extreme level. @ghidraAddress 0x12a6bc (getter), 0x12a6cc (setter) */
@property(nonatomic, assign) char levelExt;
/** The base-chart basic score. @ghidraAddress 0x12a6dc (getter), 0x12a6ec (setter) */
@property(nonatomic, assign) int scoreBas;
/** The base-chart advanced score. @ghidraAddress 0x12a6fc (getter), 0x12a70c (setter) */
@property(nonatomic, assign) int scoreAdv;
/** The base-chart extreme score. @ghidraAddress 0x12a71c (getter), 0x12a72c (setter) */
@property(nonatomic, assign) int scoreExt;
/** The base-chart basic full-combo flag. @ghidraAddress 0x12a73c (getter), 0x12a74c (setter)
 */
@property(nonatomic, assign) BOOL fullComboBas;
/** The base-chart advanced full-combo flag. @ghidraAddress 0x12a75c (getter), 0x12a76c
 * (setter) */
@property(nonatomic, assign) BOOL fullComboAdv;
/** The base-chart extreme full-combo flag. @ghidraAddress 0x12a77c (getter), 0x12a78c
 * (setter) */
@property(nonatomic, assign) BOOL fullComboExt;
/** The base-chart basic music bar. @ghidraAddress 0x12a79c (getter), 0x12a7ac (setter) */
@property(nonatomic, strong, nullable) NSData *mbarBas;
/** The base-chart advanced music bar. @ghidraAddress 0x12a7c0 (getter), 0x12a7d0 (setter) */
@property(nonatomic, strong, nullable) NSData *mbarAdv;
/** The base-chart extreme music bar. @ghidraAddress 0x12a7e4 (getter), 0x12a7f4 (setter) */
@property(nonatomic, strong, nullable) NSData *mbarExt;
/** Whether the device is an iPad. @ghidraAddress 0x12a808 (getter), 0x12a818 (setter) */
@property(nonatomic, assign) BOOL isPad;
/** Whether the device is a retina phone. @ghidraAddress 0x12a828 (getter), 0x12a838 (setter)
 */
@property(nonatomic, assign) BOOL isRetina;
/** Whether play has started. @ghidraAddress 0x12a848 (getter), 0x12a858 (setter) */
@property(nonatomic, assign) BOOL isStarted;
/** Whether the host share is startable. @ghidraAddress 0x12a868 (getter), 0x12a878 (setter)
 */
@property(nonatomic, assign) BOOL isSharedStartable;
/** The extend-chart tune. @ghidraAddress 0x12a888 (getter), 0x12a898 (setter) */
@property(nonatomic, strong, nullable) TuneInfo *extendInfo;
/** The extend-chart basic level. @ghidraAddress 0x12a8ac (getter), 0x12a8bc (setter) */
@property(nonatomic, assign) char extendLevelBas;
/** The extend-chart advanced level. @ghidraAddress 0x12a8cc (getter), 0x12a8dc (setter) */
@property(nonatomic, assign) char extendLevelAdv;
/** The extend-chart extreme level. @ghidraAddress 0x12a8ec (getter), 0x12a8fc (setter) */
@property(nonatomic, assign) char extendLevelExt;
/** The extend-chart basic score. @ghidraAddress 0x12a90c (getter), 0x12a91c (setter) */
@property(nonatomic, assign) int extendScoreBas;
/** The extend-chart advanced score. @ghidraAddress 0x12a92c (getter), 0x12a93c (setter) */
@property(nonatomic, assign) int extendScoreAdv;
/** The extend-chart extreme score. @ghidraAddress 0x12a94c (getter), 0x12a95c (setter) */
@property(nonatomic, assign) int extendScoreExt;
/** The extend-chart basic full-combo flag. @ghidraAddress 0x12a96c (getter), 0x12a97c
 * (setter) */
@property(nonatomic, assign) BOOL extendFullComboBas;
/** The extend-chart advanced full-combo flag. @ghidraAddress 0x12a98c (getter), 0x12a99c
 * (setter) */
@property(nonatomic, assign) BOOL extendFullComboAdv;
/** The extend-chart extreme full-combo flag. @ghidraAddress 0x12a9ac (getter), 0x12a9bc
 * (setter) */
@property(nonatomic, assign) BOOL extendFullComboExt;
/** The extend-chart basic music bar. @ghidraAddress 0x12a9cc (getter), 0x12a9dc (setter) */
@property(nonatomic, strong, nullable) NSData *extendMbarBas;
/** The extend-chart advanced music bar. @ghidraAddress 0x12a9f0 (getter), 0x12aa00 (setter)
 */
@property(nonatomic, strong, nullable) NSData *extendMbarAdv;
/** The extend-chart extreme music bar. @ghidraAddress 0x12aa14 (getter), 0x12aa24 (setter)
 */
@property(nonatomic, strong, nullable) NSData *extendMbarExt;
/** The owning music-select controller. @ghidraAddress 0x12aa38 (getter), 0x12aa58 (setter)
 */
@property(nonatomic, weak, nullable) MusicSelectViewController *controller;
/** The pack-id search sheet. @ghidraAddress 0x12aa6c (getter), 0x12aa7c (setter) */
@property(nonatomic, strong, nullable) SearchPackIDView *searchPackView;
/** The selected social service type. @ghidraAddress 0x12aa90 (getter), 0x12aaa0 (setter) */
@property(nonatomic, assign, nullable) NSString *socialType;
/** The recommend dimming cover. @ghidraAddress 0x12aab0 (getter), 0x12aac0 (setter) */
@property(nonatomic, strong, nullable) UIView *topcover;

@end

/**
 * The selectors the base declares but only the concrete theme subclasses implement.
 */
@interface MusicDetailView (ThemeOverrides)

/**
 * Declared, never implemented on the base: the concrete theme subclasses supply the body.
