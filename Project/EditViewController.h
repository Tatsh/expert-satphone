...
/**
 * Initialises the editor: reads the saved adjust-sector offset, resolves the device idiom,
...
/**
 * Resolves (creating if needed) the on-disk directory for a tune's edit charts.
...
/**
 * Loads the note data for the current tune from @c EditDataManager into the editor state.
 * @ghidraAddress 0x219a14
 */
- (void)loadNotesData;

/**
 * Imports a downloaded chart file into the editor, merging its metadata over the current
 *        editor info.
...
/**
 * Bumps the auto-save counter and saves the chart when it overflows or when forced.
...
/**
 * Persists the current chart, its editor info, and its summary data to disk.
 * @ghidraAddress 0x21a1d8
 */
- (void)saveNotesData;

/**
 * Resets the editor's per-session state to its defaults.
 * @ghidraAddress 0x21a94c
 */
- (void)resetEditMember;

/**
 * Sets a grid button's image and frame from a division type and origin.
...
/**
 * Builds and lays out the editor's OpenGL view and button bar.
 * @ghidraAddress 0x21ae3c
 */
- (void)loadView;

/**
 * Loads the button images, renderer, sequence, audio, and artwork for the current chart.
 * @ghidraAddress 0x21bb44
 */
- (void)loadResources;

/**
 * Tears down the renderer, sequence, framebuffer, and audio resources.
 * @ghidraAddress 0x21d38c
 */
- (void)releaseResources;

/**
 * Discarded persistence stub kept for the interface.
...
/**
 * Stops the per-frame display link.
 * @ghidraAddress 0x21d654
 */
- (void)stopAnimation;

/**
 * Handles the BGM-finished notification by clearing the playing flag.
...
/**
 * Adds notes to the sequence at every held panel, warning when the marker limit is hit.
...
/**
 * Deletes notes from the sequence at every held panel.
 * @ghidraAddress 0x21d92c
 */
- (void)deleteNote;

/**
 * Stops BGM playback and restores the play button image.
 * @ghidraAddress 0x21d9e0
 */
- (void)stopMusic;

/**
 * Clamps playback to the sequence duration and stops music when it runs past the end.
 * @ghidraAddress 0x21daac
 */
- (void)controllMusic;

/**
 * The display-link callback: reads touches, updates the sequence, and draws a frame.
...
/**
 * Plays the clap sound effect when the sequence passes a clap point.
 * @ghidraAddress 0x21fab0
 */
- (void)exeClap;

/**
 * Starts play mode: re-reads the adjust offset, resets the sequence, and applies prefs.
 * @ghidraAddress 0x21fbd8
 */
- (void)startGame;

/**
 * Toggles BGM playback from the pause button.
...
/**
 * Hides the paste button.
 * @ghidraAddress 0x21ff3c
 */
- (void)deletePasteBtn;

/**
 * Sets the mode button's background image from an image resource name.
...
/**
 * Toggles between the two edit modes.
...
/**
 * Presents the add/delete mode popover on a long press of the mode button.
...
/**
 * Applies the pinch gesture's scale to the renderer.
...
/**
 * Applies an undo and auto-saves.
...
/**
 * Presents the system-menu popover from the system-menu button.
...
/**
 * Presents the grid-division popover from the grid button.
...
/**
 * Applies a grid-division type, updating the button image and the renderer.
 * @param type The grid-division type.
...
/**
 * Enables or disables the editor buttons and renderer panel during a long press.
...
/**
 * Copies the selected area into the paste buffer.
...
/**
 * Pastes the paste buffer at the paste sector, warning on conflict.
...
/**
 * Pastes the paste buffer at the current sector.
 * @param sender The paste button.
...
/**
 * Toggles the clap sound effect on or off.
...
/**
 * Presents the how-to help popover.
...
/**
 * Rewinds playback to the current measure start.
...
/**
 * Builds the timeline scrubbing view and its gesture recognisers.
 * @ghidraAddress 0x221840
 */
- (void)createTimeLineView;

/**
 * Reads the swipe direction (result discarded).
...
/**
 * Resets the paste selection.
 * @ghidraAddress 0x221b04
 */
- (void)resetPaste;

/**
 * Resets the long-press area selection.
 * @ghidraAddress 0x221b9c
 */
- (void)resetLongPress;

/**
 * Resets both the paste and long-press selections.
 * @ghidraAddress 0x221c48
 */
- (void)resetGesture;

/**
 * Begins an area selection on a long press of the timeline.
 * @param gesture The long-press recogniser.
...
/**
 * Resets the current selection on a single tap of the timeline.
...
/**
 * Positions the paste button at the double-tapped sector.
...
/**
 * Resumes animation and BGM when returning from a paused popover.
 * @ghidraAddress 0x222298
 */
- (void)resumeInPauseView;

/**
 * Ends the edit session and returns to the root controller from a paused popover.
 * @ghidraAddress 0x222380
 */
- (void)endInPauseView;

/**
 * Dismisses the editor and ends the session.
 * @ghidraAddress 0x222474
 */
- (void)end;

/**
 * Suspends the editor when backgrounded, saving if playing.
 * @ghidraAddress 0x2224b0
 */
- (void)suspend;

/**
 * Resumes the editor's animation.
 * @ghidraAddress 0x222594
 */
- (void)resume;

/**
 * Fully resets the editor to a clean state, releasing per-chart resources.
 * @ghidraAddress 0x2225a0
 */
- (void)terminate;

/**
 * Releases the system-menu popover reference.
 * @ghidraAddress 0x222b4c
 */
- (void)templateRelease;

/**
 * The system-menu load-slot selection callback.
...
/**
 * The system-menu exit selection callback.
 * @ghidraAddress 0x222d78
 */
- (void)selectExit;

/**
 * Presents the save-confirmation alert, warning about conflicting markers.
 * @ghidraAddress 0x222db4
 */
- (void)selectSave;

/**
 * The file-list cancel callback (no-op).
 * @ghidraAddress 0x222fd4
 */
- (void)editFileListViewCancel;

/**
 * The file-list decide callback (no-op).
...
/**
 * Clears the ignore-press flag when a popover is dismissed.
...
/**
 * The editor-button popover selection callback.
...
/**
 * The confirmation-alert button callback: routes save, load, and exit confirmations.
...
/**
 * Saves the edited metadata after the modal reports a save.
 * @ghidraAddress 0x2233f4
 */
- (void)editModalViewDelegateSaveEditFile;

/**
 * Applies the adjust-time offset, clamped to zero.
...
/**
 * Removes the adjust-time offset, clamped to zero.
...
/// The clap-toggle button.
@property(nonatomic, strong, nullable) UIButton *btnClap;
/// The clap button's decorative image view.
