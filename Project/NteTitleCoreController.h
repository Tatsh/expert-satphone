...
/**
 * The animated NTE title-screen core: GL wave background, tappable concierge, hidden
...
/**
 * Sets the @c unsealHeight ivar: the top strip's height, used to offset the marker view and
...
/**
 * Initialises the controller with the background artwork names and the GL background bounds.
...
/**
 * Loads and decrypts the page background image at @p index.
...
/**
 * Becomes first responder so motion (shake) events are delivered.
...
/**
 * Builds the view hierarchy: the GL view, the background image view, the option ornament,
...
/**
 * Starts the title: hides the logo and prompt, loads and plays the BGM and welcome SE, and
...
/**
 * Launches the looping "run car" flyby of the option ornament across the screen.
 * @ghidraAddress 0x1cf408
 */
- (void)startRunCar;

/**
 * The @c CADisplayLink callback: renders the GL frame and feeds accelerometer deltas into
 * the wave background.
...
/**
 * Installs the swipe and tap recognisers, resets the easter-egg state, and starts the
 * blinking prompt.
...
/**
 * Fades the jubeat logo up to full opacity, then begins the marker check on completion.
...
/**
 * The option ornament's current presentation-layer frame (its live animated position).
...
/**
 * Drops a pair of falling option ornaments at the tap point.
...
/**
 * The tap-gesture handler: drives the Konami-code hit regions, the concierge ripple, the
...
/**
 * Runs the page-curl transition to a new background page.
...
/**
 * The reverse-curl animation completion: curls the current page back into view.
 * @ghidraAddress 0x1d0fd0
 */
- (void)needReverseEnd;

/**
 * The curl-animation completion: re-enables interaction events.
...
/**
 * Invalidates the loop, stops device motion, and stops all sound effects.
 * @ghidraAddress 0x1d12e8
 */
- (void)stopAnimation;

/**
 * Tears down the option, background, prompt, recogniser, and texture references.
 * @ghidraAddress 0x1d1388
 */
- (void)viewDidUnload;

/**
 * An option-drop finished falling: removes it from the view and the tracking array.
...
/**
 * Whether the given (legacy) interface orientation is supported: landscape only.
...
/**
 * The supported interface orientations: landscape.
...
/**
 * Removes observers, stops the option view and device motion, terminates the SE player,
 * and calls @c super.
...
/**
 * @c EditorIDManagerDelegate / licence error: shows the network-error alert and tears down
 * the sheet and cover.
...
/**
 * Whether the given motion event is a shake.
...
/**
 * Whether the controller can become first responder: always @c YES.
 * @return @c YES.
...
/**
 * Motion began: routes to the shake check.
...
/**
 * Motion cancelled: routes to the shake check.
 * @param motion The motion type.
 * @param event The motion event.
...
/**
 * Motion ended: routes to the shake check and plays the shake SE when it was a shake.
 * @param motion The motion type.
 * @param event The motion event.
