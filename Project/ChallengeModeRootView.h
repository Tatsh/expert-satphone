...
/**
 * @brief The scratch challenge-mode root container.
 *
...
/**
 * @brief The shared modal progress/store dialog panel.
 * @ghidraAddress 0x71bfc (getter)
 */
@property(nonatomic, strong, readonly, nullable) StoreDialogView *modalDialog;

/**
 * @brief Builds the whole challenge-mode hierarchy over the main-screen bounds.
...
/**
 * @brief Prefixes a sound-effect base name with the current theme's prefix.
...
/**
 * @brief Presents the appropriate login/information/how-to sheet or the login message.
 * @ghidraAddress 0x69050
 */
- (void)showLoginMessage;

/**
 * @brief Checks whether every line-up artwork is present on disk, queueing the rest for download.
...
/**
 * @brief Starts the next queued artwork download.
 * @ghidraAddress 0x6a114
 */
- (void)imageDownload;

/**
 * @brief Shows or hides the present-notification badge from the current present count.
 * @ghidraAddress 0x6a244
 */
- (void)setNotificateImage;

/**
 * @brief Slides the challenge view in and starts the event BGM.
...
/**
 * @brief Slides the challenge view off the top edge and closes challenge mode.
...
/**
 * @brief Seeds the shared status from a challenge-data dictionary and starts the refresh timer.
...
/**
 * @brief Iterates the line-up records (the binary discards each parsed value).
 * @ghidraAddress 0x6aaa0
 */
- (void)deleteMusicData;

/**
 * @brief Refreshes the badge and the scratch board.
 * @ghidraAddress 0x6abbc
 */
- (void)refreshView;

/**
 * @brief Closes the challenge session after a server session error.
 * @ghidraAddress 0x6ac08
 */
- (void)closeChallengeModeSessionError;

/**
 * @brief The close-button action: plays the close sound and leaves challenge mode.
...
/**
 * @brief The menu-button action: presents the challenge menu root view.
...
/**
 * @brief The line-up-button action: presents the line-up view over a cover.
...
/**
 * @brief The ranking-button action: opens the all-songs ranking.
...
/**
 * @brief Calls @c -boolValue on the flag and discards the result.
...
/**
 * @brief A no-op in the shipped binary.
 * @param dispFlag The boxed display flag.
...
/**
 * @brief Routes an alert result to the matching scratch, purchase, or close action.
...
/**
 * @brief Opens the per-panel music detail sheet with a scale-in animation.
 * @ghidraAddress 0x6b9a8
 */
- (void)openDetailView;

/**
 * @brief Closes the music detail sheet with a scale-out animation.
 * @ghidraAddress 0x6bf34
 */
- (void)closeDetailView;

/**
 * @brief Closes the name-setting sheet, then shows the login message.
 * @ghidraAddress 0x6c24c
 */
- (void)closeMenu;

/**
 * @brief Removes the challenge menu root view.
 * @ghidraAddress 0x6c420
 */
- (void)closeMenuView;

/**
 * @brief Refreshes the detail sheet after a ranking change.
 * @ghidraAddress 0x6c45c
 */
- (void)changeRanking;

/**
 * @brief Builds the scratch-confirmation message, warning when little play time remains.
...
/**
 * @brief Starts the challenge music, recovering play coins with cubes when short.
 * @ghidraAddress 0x6c59c
 */
- (void)startChallengeMusic;

/**
 * @brief Applies the result of a nail (single-panel) scratch.
...
/**
 * @brief Fades the menu cover in.
 * @ghidraAddress 0x6cc9c
 */
- (void)showMenuCoverView;

/**
 * @brief Fades the menu cover out and removes it.
 * @ghidraAddress 0x6cdec
 */
- (void)hideMenuCoverView;

/**
 * @brief Fades the modal cover in.
 * @ghidraAddress 0x6cfa4
 */
- (void)showCoverView;

/**
 * @brief Fades the modal cover out and removes it.
 * @ghidraAddress 0x6d0f4
 */
- (void)hideCoverView;

/**
 * @brief Shows the modal dialog as a bare progress panel with the given message.
...
/**
 * @brief Hides the purchase progress dialog.
 * @ghidraAddress 0x6d59c
 */
- (void)hidePurchaseDialog;

/**
 * @brief Sets the modal dialog's message text.
...
/**
 * @brief Shows the modal dialog wired to a delegate, with the abort button enabled on completion.
...
/**
 * @brief Hides the modal dialog.
 * @ghidraAddress 0x6dd88
 */
- (void)hideModalDialog;

/**
 * @brief The per-tick refresh: updates the status bar and, once per session, the login message.
...
/**
 * @brief Handles a finished session/artwork download.
...
/**
 * @brief Handles a failed session/artwork download.
...
/**
 * @brief Presents the cube-purchase view.
 * @ghidraAddress 0x6f9f4
 */
- (void)cubePurchaseStart;

/**
 * @brief Dismisses the cube-purchase view.
 * @ghidraAddress 0x6fcf4
 */
- (void)closeCubePurchase;

/**
 * @brief The cover-tap action: dismisses whichever sub-view is showing.
 * @ghidraAddress 0x6fecc
 */
- (void)closeView;

/**
 * @brief Dismisses the line-up view.
 * @ghidraAddress 0x6ff50
 */
- (void)closeLineupView;

/**
 * @brief Dismisses the login message.
 * @ghidraAddress 0x6ff9c
 */
- (void)closeLoginMessage;

/**
 * @brief Dismisses the login information sheet.
 * @ghidraAddress 0x70008
 */
- (void)closeLoginInformation;

/**
 * @brief The StoreKit success callback.
...
/**
 * @brief The StoreKit failure callback.
...
/**
 * @brief Selects a scratch panel and confirms the scratch.
...
/**
 * @brief Enables or disables scratching.
...
/**
 * @brief Whether scratching is currently enabled.
...
/**
 * @brief Begins a scratch on the given panel.
...
/**
 * @brief Opens the scratch-complete view.
 * @ghidraAddress 0x70e88
 */
- (void)openScratchComplete;

/**
 * @brief Closes the scratch-complete view.
 * @ghidraAddress 0x70fc4
 */
- (void)closeScratchComplete;

/**
 * @brief Ends a scratch on the given panel.
 * @param sender The scratch view.
...
/**
 * @brief Opens the all-songs ranking over a cover.
 * @ghidraAddress 0x7100c
 */
- (void)openAllRanking;

/**
 * @brief Opens the ranking for the selected panel, cross-fading from the detail sheet.
 * @ghidraAddress 0x71358
 */
- (void)openRanking;

/**
 * @brief Closes the ranking, cross-fading back to the detail sheet when appropriate.
 * @ghidraAddress 0x71730
 */
- (void)closeRanking;

/**
 * @brief Enters the jubeat store's pack-purchase flow for the given pack.
...
/**
 * @brief Refreshes the status bar display.
