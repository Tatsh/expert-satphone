...
| --- | --- | --- |
| `-[StorePackCell initWithStyle:reuseIdentifier:]` | 0xf0e48 | The one open partial. About 652 instructions (0xf0e48 to 0xf1878) and the only member that builds anything: it must produce all six ivar subviews — `bgView`, `labelName`, `labelPrice`, `labelPurchased`, `newMarker`, `extendMarker` — plus the `_artworkView` behind the read-only property. The class is a near-twin of `StoreRecommendPackView`, whose `-initWithFrame:` (0x1449fc) is reconstructed and is the closest reference; expect the same shape with a `UITableViewCell` `contentView` instead of a plain `UIView`. Its ivar names match one for one. |
