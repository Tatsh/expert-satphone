...
which of the four subcommands actually cover anything here. Last run: **191 annotated, 0
mismatched, 0 selectors absent; 48 constants checked against their bytes.**
...
**As of the last run: 186 of 5036 methods, 3.7%. 58 of 317 classes complete.**
...
| `Project/ChallengeMenuViewCell.m` | Three of four methods. `-setBgImage:numImage:` is declared only — see Next. |
...
| --- | --- | --- |
| `-[ChallengeMenuViewCell setBgImage:numImage:]` | 0x41c24 | Three of four members written; this one is left. About 444 instructions, and it is the only member that builds subviews — the initialiser creates none. It must produce `menuBtn`, `numImageBg`, the three `numImage3d` slots and the two `numImage2d` slots, and store the digit array into `numImage`. Note `-tapMenu:` is an empty `ret`, so whether the button targets it at all is worth confirming from this body. |
