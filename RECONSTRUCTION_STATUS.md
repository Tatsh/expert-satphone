...
which of the four subcommands actually cover anything here. Last run: **356 annotated, 0
...
**As of the last run: 345 of 5036 methods, 6.9%. 95 of 317 classes complete.**
...
| `Project/NotificationPageNavController.m` | Five of six methods. `-init:` is declared only — see Next. |
...
| --- | --- | --- |
| `-[NotificationPageNavController init:]` | 0x182b8c | The one open partial. About 272 instructions, and the only member that builds anything: it must produce the `pageViewController` and the `notificationPage` web view, and set the weak `delegate` that `-pushClose:` reads back. The argument is untyped in the metadata (`@24@0:8@16`). `NotificationPageViewController` is a declared-only stub named from the ivar encoding; expect this method to fill it in. |
