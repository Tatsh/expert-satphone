...
| `ApplilinkCore` network methods, `ApplilinkConsts (appliId/version/setUserId:)`, and `RewardCore`/`RecommendCore`/`ApplilinkViewManager sharedInstance/rotate…` | `+[ApplilinkNetwork …]` forwards every facade to them                                                | not located yet              |
